...
const SHACLReportViewer: React.FC<SHACLReportViewerProps> = ({ 
  ...
  const renderProfileName = () => {
    const profileName = t(`validation.profiles.${report.profile}`);
    ...
    if (profileUrl) {
      return (
        <a 
          ...
          title={t('shacl.actions.view_profile_documentation')}
        >
    ...
    return (
      <>
        {entities.map((entity, index) => (
          <span key={index} className="badge bg-info ms-2" title={`${t('shacl.details.entity_context')}: ${entity}`}>
        ...
        {overflowCount > 0 && (
          <span 
            ...
            title={`${t('shacl.results.additional_entities', { count: overflowCount })}`}
          >
            +{overflowCount}
          </span>
        )}
  ...
  const formatPath = (path?: string) => {
    if (!path) return t('shacl.details.no_path');
    ...
    if (isURL(text)) {
      return (
        <a 
          ...
          title={t('shacl.actions.open_link')}
        >
          {text}
          <i className="bi bi-box-arrow-up-right ms-1" style={{ fontSize: '0.75em' }}></i>
        </a>
            ...
            <div className="flex-grow-1">
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        ...
                        <button
                          ...
                          title={isExpanded ? t('shacl.actions.collapse_details') : t('shacl.actions.expand_details')}
                        >
              ...
              {isExpanded && (
                <div className="mt-3 border-top pt-3">
                  <h6 className="mb-2">
                    ...
                    {t('shacl.details.affected_nodes')} ({group.count})
                  </h6>
                    ...
                    {group.violations.map((violation, vIndex) => (
                      <div key={vIndex} className="col-12">
                        <div className="card card-body p-2">
                          {violation.focusNode && (
                            <div className="mb-1">
                              <strong>{t('shacl.details.focus_node')}:</strong>{' '}
                          ...
                          {violation.entityContext && violation.entityContext !== 'Unknown Entity' && (
                            <div className="mb-1">
                              <strong>{t('shacl.details.entity_context')}:</strong>{' '}
                          ...
                          {violation.path && (
                            <div className="mb-1">
                              <strong>{t('shacl.details.path')}:</strong>{' '}
                          ...
                          {violation.value && (
                            <div className="mb-1">
                              <strong>{t('shacl.details.value')}:</strong>{' '}
                          ...
                          {violation.sourceShape && (
                            <div className="mb-1">
                              <strong>{t('shacl.details.source_shape')}:</strong>{' '}
                  ...
                  <div className="col-md-6">
                    <div className="text-center">
                      ...
                      <h4 className={`mt-2 ${getComplianceColor(report.conforms)}`}>
                        {report.conforms ? t('shacl.status.conforms') : t('shacl.status.non_conforms')}
                      </h4>
                    ...
                    <div className="row text-center">
                      <div className="col-4">
                        <div className="border rounded p-3">
                          ...
                          <small className="text-muted">{t('shacl.severity.violations')}</small>
                        </div>
                      </div>
                      <div className="col-4">
                        <div className="border rounded p-3">
                          <div className="h4 text-warning mb-1">{report.warnings.length}</div>
                          <small className="text-muted">{t('shacl.severity.warnings')}</small>
                        </div>
                      </div>
                      <div className="col-4">
                        <div className="border rounded p-3">
                          <div className="h4 text-info mb-1">{report.infos.length}</div>
                          <small className="text-muted">{t('shacl.severity.infos')}</small>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {totalIssues > 0 ? (
                  <>
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <h6 className="mb-0">
                        {t('shacl.results.showing_results', { count: filteredViolations.length, total: totalIssues })}
                      </h6>
                      <div className="d-flex align-items-center gap-2">
                        <div className="d-flex align-items-center">
                          <label htmlFor="severityFilter" className="form-label mb-0 me-2">
                            {t('shacl.severity.filter_by_severity')}:
                          </label>
                          <select
                            id="severityFilter"
                          ...
                          >
                            ...
                            <option value="Warning">{t('shacl.severity.warnings')} ({report.warnings.length})</option>
                            <option value="Info">{t('shacl.severity.infos')} ({report.infos.length})</option>
                          </select>
                        </div>
                        <div className="btn-group btn-group-sm">
                          <button 
                            ...
                            title={t('shacl.actions.expand_all_groups')}
                          >
                            <i className="bi bi-arrows-expand me-1"></i>
                            {t('shacl.actions.expand_all')}
                          </button>
                          <button 
                            className="btn btn-outline-secondary"
                            onClick={collapseAllMessages}
                            title={t('shacl.actions.collapse_all_groups')}
                          >
                            <i className="bi bi-arrows-collapse me-1"></i>
                            {t('shacl.actions.collapse_all')}
                          </button>
                        </div>
                      </div>
                    </div>
                        ...
                        if (groupedViolations.length === 0) {
                          return (
                            <div className="text-center text-muted py-4">
                              ...
                              <p className="mt-2">{t('shacl.results.no_results_for_filter')}</p>
                            </div>
                          );
                        }
                  ...
                  <div className="text-center text-muted py-5">
                    ...
                    <h4 className="mt-3 text-success">{t('shacl.status.perfect_compliance')}</h4>
                    <p className="text-muted">
                      {t('shacl.status.no_violations_found')}
                    </p>
                  </div>
                )}
              </div>
